...
function SVMClassification() {
  ...
  const SVMClassificationCall = async event => {
      ...
      try{
          const response = await axios.post('http://127.0.0.1:8000/api/handleSVMClassification/', formData, {
              headers: { 'Content-Type': 'multipart/form-data' }
            });
          console.log("Successfully Trained SVM Classification");
      }catch(error){
        console.log(error);
      }
...
return (
  <div>
      ...
      <Box
        ...
        noValidate
        autoComplete="off"
        onSubmit={SVMClassificationCall} 
      >
...
export default SVMClassification
