...
function LassoRegression() {
    ...
    const LassoRegressionCall = async event => {
        ...
        try{
            const response = await axios.post('http://127.0.0.1:8000/api/handleLassoRegression/', formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
              });
            console.log("Successfully Trained Lasso Regression")
        }catch(error){
          console.log(error);
        }
  ...
  return (
    <div>
      ...
      <Box
        ...
        noValidate
        autoComplete="off"
        onSubmit={LassoRegressionCall} 
      >
        <TextField id="outlined-basic" label="Alpha" variant="outlined" onChange={e => setAlpha(e.target.value)}/>
        <br></br>
